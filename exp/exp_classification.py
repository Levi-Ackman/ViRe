...
class Exp_Classification(Exp_Basic):
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        self.model.eval()
        with torch.no_grad():
            for i, (batch_x, label, vlm_emb) in enumerate(vali_loader):
                batch_x = batch_x.float().to(self.device)
                vlm_emb = vlm_emb.float().to(self.device)
                label = label.to(self.device)

                outputs = self.model(batch_x,vlm_emb)
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, label, vlm_emb) in enumerate(train_loader):
                iter_count += 1
