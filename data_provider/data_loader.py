...
class APAVALoader(Dataset):
    def __init__(self, root_path, flag=None):
        ...
        self.emb_VLM_path = f"./emb_VLM/APAVA/{flag}/"
    ...
    def __getitem__(self, index):
        ...
        # Get vlm embeddings
        emb_vlm_stack = []
        emb_vlm_path = os.path.join(self.emb_VLM_path, f"{index}.h5")
        
        if os.path.exists(emb_vlm_path):
            with h5py.File(emb_vlm_path, 'r') as hf:
                data = hf['embeddings'][:]
                tensor = torch.from_numpy(data)
                emb_vlm_stack.append(tensor.squeeze(0))
        else:
            raise FileNotFoundError(f"No embedding file found at {emb_vlm_path}")       
        
        emb_vlm = torch.stack(emb_vlm_stack, dim=-1)
        
        return torch.from_numpy(self.X[index]), torch.from_numpy(np.asarray(self.y[index])), emb_vlm.squeeze(-1)
...
class TDBRAINLoader(Dataset):
    def __init__(self, root_path, flag=None):
        ...
        self.max_seq_len = self.X.shape[1]
        self.emb_VLM_path = f"./emb_VLM/TDBRAIN/{flag}/"
...
class ADFTDLoader(Dataset):
    def __init__(self, root_path, flag=None):
        ...
        self.max_seq_len = self.X.shape[1]
        self.emb_VLM_path = f"./emb_VLM/ADFTD/{flag}/"
...
class PTBLoader(Dataset):
    def __init__(self, root_path, flag=None):
        ...
        self.max_seq_len = self.X.shape[1]
        self.emb_VLM_path = f"./emb_VLM/PTB/{flag}/"
...
class PTBXLLoader(Dataset):
    def __init__(self, root_path, flag=None):
        ...
        self.max_seq_len = self.X.shape[1]
        self.emb_VLM_path = f"./emb_VLM/PTB-XL/{flag}/"
...
class MIMICIVLoader(Dataset):
    def __init__(self, root_path, flag=None):
        ...
        self.max_seq_len = self.X.shape[1]
        self.emb_VLM_path = f"./emb_VLM/MIMIC/{flag}/"
